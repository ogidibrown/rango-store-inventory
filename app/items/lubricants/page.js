...
const LubricantPage = () => {
  ...
  const fetchItems = async () => {
    const q = query(collection(db, "items"), where("category", "==", "Lubricants"));
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Lubricants</h1>
        {lowCount > 0 && (
      ...
      <ul className="space-y-3">
        ...
        {items.length === 0 && <p className="text-gray-500">No lubricant items found.</p>}
      </ul>
    </div>
  );
};

export default LubricantPage;
